 ...
 */
public class MyChannelHistoryModuleImpl implements ChannelHistoryModule {
    ...
    public MyChannelHistoryModuleImpl(SlackSession session) {
    ...
    private List<SlackMessagePosted> fetchHistoryOfChannel(Map<String, String> params, String command) {
        ...
        if (events != null) {
            for (Object event : events) {
                if ((((JSONObject) event).get("subtype") == null) || (((JSONObject) event).get("subtype").equals("bot_message"))) {
                    messages.add((SlackMessagePosted) MySlackJSONMessageParser.decode(session, (JSONObject) event));
                    Log.i("SLACKEVENTO", ((JSONObject) event).toString());
                }
            }
        }
        return messages;
